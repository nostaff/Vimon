...
module.exports = function generateEntry (fileNames) {
  return new Promise((resolve, reject) => {
    ...
    let startcaseNames = []
    let importString = '/** This file is generated by ./build/lib-builder/build-entry.js */\n\n'
    fileNames.forEach(function (item) {
      let _startcase = startcase(item).split(' ').join('')
      // let _importString = `export { default as ${_startcase} } from './components/${item}'\n`
      let _importString = `import ${_startcase} from './${item}/index.js'\n`
    ...
    // `
    importString += `\nexport {\n  `
    importString += startcaseNames.join(',\n  ')
    // startcaseNames.forEach(function (startcaseName) {
    //   importString += `${startcaseName},\n`
    // })
    importString += `\n}`

    // fs.createWriteStream(`${config.srcPath}/index.js`)
    // fs.writeFile(`${config.srcPath}/index.js`, importString, function (err) {
    fs.createWriteStream(`${config.componentPath}/index.js`)
    fs.writeFile(`${config.componentPath}/index.js`, importString, function (err) {
      if (err) {
        reject(err)
        throw err
      }
