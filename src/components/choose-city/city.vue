  ...
  <vm-content class="outer-content ion-city-selector" ref="content">
    <div class="city-selector">
      ...
      <div class="city-selector__group" v-if="showHotCities && hotCityList.length>0">
        ...
        <div class="city-selector__group--hotBox">
          <div class="hotBox__wrap" @click="selectCity(item)" v-for="item in hotCityList" :key="item.adCode">
      ...
      <div class="city-selector__group" v-for="(classify,index) in cityList" :key="index">
        ...
        <div class="city-selector__group--item" v-for="item in classify.cities" :key="item.adCode" @click="selectCity(item)">
          <div>{{item.city}}</div>
        </div>
      </div>
    </div>
    ...
    getCurrentLocation () {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof window.AMap === 'undefined') {
          getScript(
            `//webapi.amap.com/maps?v=1.3&key=${this.$options.$data.ak}`
          ).then(
            () => {
