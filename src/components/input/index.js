/**
  ...
  * @props {Boolean} [autofocus]               - 是否自动获得焦点
  * @props {String} [autocapitalize]           - 是否开启首字母大写: "on", "off", "words", "characters", "sentences"
  * @props {String} [autocorrect]              - 是否开启自动更正: "on", "off"
  * @props {String} [autocomplete]             - 是否开启自动完成: "on", "off"
  ...
  * @props {Boolean} [valid]                   - 是否valid输入结果, 如果regex有值, 则开启, 否则关闭. 如果valid开启, 但是regex无值, 则使用内置判断. 默认关闭valid, valid只是作为内部正误标示, 对外提交不起作用, 如果点击能知道各个input的状态, 需要在dom中search'ng-invalid'类名, 这样的话, 验证位置就会统一.
  *
