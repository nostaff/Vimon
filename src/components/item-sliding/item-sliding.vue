  ...
  methods: {
    init () {
      ...
      this.$children.forEach(item => {
        if (item.hasClass('ion-item')) {
          this.itemCmp = item
        }
        if (item.hasClass('ion-item-options') && item.side === 'left') {
          this.leftOptions = item
          side |= this._getSides(item)
        }
        if (item.hasClass('ion-item-options') && item.side === 'right') {
      ...
      //  事件注册
      this.$events &&
        this.$events.$on('onItemSlidingOpen', ins => {
          if (this !== ins) {
            this.close()
          }
        })
    ...
    _isHorizontalSliding (start, end) {
      ...
      return (
        Math.abs(deltaX) > MAX_DELTAX && Math.abs(deltaX) > Math.abs(deltaY)
      )
    },
    ...
    _calculateOptsWidth () {
      ...
      if (this.rightOptions) {
        ...
        console.assert(
          this.optsWidthRightSide > 0,
          'optsWidthRightSide should not be zero'
        )
      }
      ...
      if (this.leftOptions) {
        ...
        console.assert(
          this.optsWidthLeftSide > 0,
          'optsWidthLeftSide should not be zero'
        )
      }
    ...
    _setState (state) {
      ...
      this.setElementClass(
        ...
        !!(state & SLIDING_STATE.Right)
      )
      this.setElementClass(
        'active-options-left',
        !!(state & SLIDING_STATE.Left)
      )
      this.setElementClass(
        'active-swipe-right',
        !!(state & SLIDING_STATE.SwipeRight)
      )
      this.setElementClass(
        'active-swipe-left',
        !!(state & SLIDING_STATE.SwipeLeft)
      )
      this.state = state
    }
  }
